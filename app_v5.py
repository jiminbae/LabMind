...
def apply_theme() -> None:
    st.markdown(
        ...
        .st-key-primary_view button[data-variant="segmented_control"] p,
        .st-key-primary_view div[data-testid="stSegmentedControl"] button p {
            font-family: inherit !important;
        ...
        .st-key-query_mode button[data-variant="segmented_control"] p,
        .st-key-query_mode div[data-testid="stSegmentedControl"] button p {
            font-family: inherit !important;
            font-size: 1em !important;
            font-stretch: normal !important;
            font-weight: 700 !important;
...
def render_upload_step(*, compact: bool = False) -> bytes | None:
    ...
    if compact and uploaded_file is not None:
        ...
        if st.button(
            ...
            with st.spinner("Reading the reagent label with Gemini…", show_time=True):
    ...
    if uploaded_file is None:
        st.markdown(
        '<p class="upload-hint" role="note">Use a straight-on, glare-free photo with the chemical name, CAS number, lot, and manufacturer in view.</p>',
            unsafe_allow_html=True,
        )
        return None
...
def render_extraction_step() -> None:
    ...
    render_section_header(
        "Step 2",
        "Review the extracted identity",
        "Confirm each AI-read value and complete anything the reagent label did not show.",
    )
    ...
    if extraction_notice:
        ...
        if extraction_notice.get("status") == "failed":
            ...
            st.success(message)
    st.markdown(
        '<div class="status-line"><span class="status-dot"></span>AI suggestions remain editable until registration</div>',
        unsafe_allow_html=True,
    )
...
def request_chemical_classification() -> None:
    """Defer the slow provider call so its progress UI renders before the widgets."""

    st.session_state["add_classification_requested"] = True


def render_storage_step() -> None:
    if not st.session_state.get("add_extraction_complete"):
        return
    if st.session_state.pop("add_classification_requested", False):
        with st.spinner(
    ...
    st.markdown(
        ...
        <div class="safety-card">
            ...
            <div class="safety-title">AI proposes chemistry. Lab rules decide storage.</div>
            <div class="order-detail">
                Gemini may suggest multiple labels and constraints, but it cannot choose
                a cabinet or override the laboratory's segregation policy.
            </div>
        </div>
        """,
        unsafe_allow_html=True,
    ...
    with constraint_col:
        ...
        st.caption("Review every constraint before LabMind runs the storage rules.")
    ...
    if not classification["labels"] and not classification["constraints"]:
        st.button(
            "Generate AI chemistry profile",
            width="stretch",
            key="run_chemical_classification",
            disabled=not validate_cas_number(
                st.session_state.get("add_field_cas_number", "")
            ),
            on_click=request_chemical_classification,
        )
    ...
    st.markdown(
        ...
        <div class="decision-card">
            <div class="decision-route">LabMind deterministic safety rule</div>
    ...
    st.markdown(
        '<p class="quiet-note">LabMind stores the final location together with the triggered rule and reviewer approval.</p>',
        unsafe_allow_html=True,
    )
...
def render_confirmation_step() -> None:
    ...
    if st.button(
        "Add reviewed reagent to inventory",
        type="primary",
        width="stretch",
        ...
        try:
            with st.spinner("Saving the reviewed reagent record…", show_time=True):
    ...
    if confirmation:
        ...
        st.markdown(
            ...
            <div class="confirmation">
                ...
                <div class="confirmation-title">Reagent added to trusted inventory</div>
                <p class="section-copy">
                    The reviewed record is now available in LabMind Inventory search.
                </p>
            </div>
            """,
            unsafe_allow_html=True,
    ...
    *,
    ...
    if frame.empty:
        if inventory_is_empty:
            title = "Build your first trusted inventory record"
            copy = (
                "Complete Verified reagent intake, then return here to search the "
                "reviewed reagent."
            )
        else:
            title = "No reviewed inventory records match"
            copy = "Broaden the request or clear one or more constraints, then verify again."
        st.markdown(
            f"""
    ...
    st.download_button(
        "Export verified results as CSV",
...
def render_basic_query(frame: pd.DataFrame) -> pd.DataFrame:
    ...
    if search_clicked or "query_results" not in st.session_state:
        with st.spinner("Checking the loaded inventory records…"):
...
def render_natural_language_query(frame: pd.DataFrame) -> pd.DataFrame:
    ...
    if st.button(
        ...
        with st.spinner(
            "Interpreting the question and verifying inventory…",
            show_time=True,
        ):
